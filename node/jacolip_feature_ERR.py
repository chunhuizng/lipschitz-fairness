...
def avg_ndcg(x_corresponding, x_similarity, x_sorted_scores, y_ranks, top_k):
    ...
    wandb.log({"Average NDCG@k": avg_ndcg.item()})
...
def train_fair(epoch, model_name, adj, output, model, features, idx_train, idx_test, top_k, all_ndcg_list_test, optimizer, sigma_1, lambdas_para=1):
    model.train()
    ...
    avg_err_result = avg_err(x_corresponding, x_similarity, x_sorted_scores, y_sorted_idxs, top_k)
    all_ndcg_list_test.append(avg_err_result)
...
def err_computer(output, idx_test, features, top_k):
...
def train_lipgrad(epoch, model_name, adj, model=None, flag=1, lip_grad = False, u=0.0, features=None, idx_train=None, idx_val=None, idx_test=None, labels=None, optimizer=None):
    t = time.time()
    model.train()
    optimizer.zero_grad()
    if model_name == 'SGC':
    ...
    for i in range(out.shape[1]):
        v = torch.zeros_like(out)
        v[:, i] = 1
        gradients = autograd.grad(outputs=out, inputs=input, grad_outputs=v, create_graph=True, retain_graph=True,
                                  only_inputs=True)[0]
        grad_norm = torch.norm(gradients, dim=1).unsqueeze(dim=1)
        lip_mat.append(grad_norm)
    ...
    if (epoch + 1) % 10 == 0:
        err_computer(output1, idx_test=idx_test, features=features, top_k=10)
    

    return output1


def train(epoch, model_name, adj, flag=0, model=None, features=None, labels=None, idx_train=None, idx_val=None, optimizer=None, idx_test=None):
    t = time.time()
    model.train()
    optimizer.zero_grad()
    if model_name == 'SGC':
    ...
    wandb.log({'train_loss': loss_train.item(), 'train_acc': acc_train.item(), 'val_loss': loss_val.item(), 'val_acc': acc_val.item()})
    if (epoch+1)%10 == 0:
        err_computer(output1, idx_test=idx_test, features=features, top_k=10)
    return output1
...
def train_model():
    ...
    wandb.init(project="fairnode-feat-err")
    args = get_citation_args()
    pprint(args)
    dataset = args.dataset  
    model_name = args.model_name  
    args.save_dir = 'log/err/' + 'feat/' + str(model_name) + '/' + str(dataset) + '/'
    ...
    for epoch in range(pre_train):
        ...
        output = train_lipgrad(epoch, model_name, adj, model=model, lip_grad=True, u=args.u_lip, features=features, optimizer=optimizer, labels=labels, idx_train=idx_train, idx_val=idx_val, idx_test=idx_test)
        

    for epoch in range(args.epochs):
        output = train(epoch, model_name, adj, model=model, features=features, labels=labels, idx_train=idx_train,
                       idx_val=idx_val, optimizer=optimizer, idx_test=idx_test)
        
        
        all_ndcg_list_test = train_fair(epoch, model_name, adj, output, model=model, features=features, idx_train=idx_train, idx_test=idx_test, top_k=top_k, all_ndcg_list_test=all_ndcg_list_test, optimizer=optimizer, lambdas_para=lambdas_para, sigma_1=sigma_1)
...
if __name__ == '__main__':
    ...
    sweep_id = wandb.sweep(sweep_config, project="fairnode-feat-err")

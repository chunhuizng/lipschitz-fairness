...
def ndcg_computer(output, idx_test, jaccard_adj_orig, top_k=10, k_para=None):

    y_similarity = simi(output[idx_test])
    x_similarity = jaccard_adj_orig[idx_test, :][:, idx_test]

    print("Ranking optimizing... ")
    x_sorted_scores, y_sorted_idxs, x_corresponding = lambdas_computation_only_review(x_similarity, y_similarity, top_k, k_para=k_para)
    avg_ndcg(x_corresponding, x_similarity, x_sorted_scores, y_sorted_idxs, top_k)



def lambdas_computation_only_review(x_similarity, y_similarity, top_k, k_para=None):
    max_num = 2000000
    x_similarity[range(x_similarity.shape[0]), range(x_similarity.shape[0])] = max_num * torch.ones_like(x_similarity[0, :])
    y_similarity[range(y_similarity.shape[0]), range(y_similarity.shape[0])] = max_num * torch.ones_like(y_similarity[0, :])
...
def train_fair(epoch, model_name, adj, jaccard_adj_orig, output, model, features, idx_train, idx_test, top_k, all_ndcg_list_test, optimizer, sigma_1, lambdas_para=1, k_para=None):
    model.train()
...
def train_lipgrad(epoch, model_name, adj, model=None, flag=1, lip_grad = False, u=0.0, features=None, idx_train=None, idx_val=None, idx_test=None, labels=None, optimizer=None, jaccard_adj_orig=None, top_k=None, k_para=None):
    t = time.time()
    model.train()
    optimizer.zero_grad()
    if model_name == 'SGC':
    ...
    if (epoch + 1) % 10 == 0:
        ndcg_computer(output1, idx_test=idx_test, jaccard_adj_orig=jaccard_adj_orig, top_k=top_k, k_para=k_para) 

    return output1

def train(epoch, model_name, adj, flag=0, model=None, features=None, labels=None, idx_train=None, idx_val=None, optimizer=None, idx_test=None, jaccard_adj_orig=None, top_k=10):
    t = time.time()
    model.train()
    optimizer.zero_grad()
    if model_name == 'SGC':
    ...
    if (epoch+1)%10 == 0:
        ndcg_computer(output1, idx_test=idx_test, jaccard_adj_orig=jaccard_adj_orig, top_k=top_k) 
...
def get_citation_args():
    ...
    return args
def train_model():
    wandb.init(project="fairnode-struct-ndcg")
    args = get_citation_args()
    dataset = args.dataset  
    model_name = args.model_name  
    args.save_dir = 'log/ndcg/' + 'struc/' + str(model_name) + '/' + str(dataset) + '/'
    ...
    for epoch in range(args.epochs):
        output = train_lipgrad(epoch, model_name, adj, model = model, flag=1, lip_grad = True, u=args.u_lip, idx_test=idx_test, idx_val=idx_val, idx_train=idx_train, features=features,
                               top_k=top_k, k_para=k_para, optimizer=optimizer, jaccard_adj_orig=jaccard_adj_orig, labels=labels)
...
if __name__ == '__main__':
    ...
    sweep_id = wandb.sweep(sweep_config, project="fairnode-struct-ndcg")

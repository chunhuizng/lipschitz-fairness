...
def train_fair(epoch, model_name, adj, output, model, features, idx_train, idx_test, top_k, all_ndcg_list_test, optimizer, sigma_1, lambdas_para=1):
    ...
    lambdas1, x_sorted_scores, y_sorted_idxs, x_corresponding = lambdas_computation(x_similarity, y_similarity1, top_k, sigma_1=sigma_1, k_para = 1) 
...
def train_model():
    ...
    for epoch in range(pre_train):
        output = train_lipgrad(epoch = epoch, model_name = model_name, adj = adj, model=model, flag=1, lip_grad=True, u=args.u_lip, features=features,
    ...
    for epoch in range(args.epochs):
        ...
        output = train(epoch = epoch, model_name = model_name, flag=0, adj = adj, model=model, features=features, labels=labels, idx_train=idx_train,
        ...
        all_ndcg_list_test = train_fair(epoch=epoch, model_name=model_name, adj=adj, output=output, 
                                        model=model, features=features, idx_train=idx_train, idx_test=idx_test, top_k=top_k, all_ndcg_list_test=all_ndcg_list_test, optimizer=optimizer, lambdas_para=lambdas_para, sigma_1=sigma_1)

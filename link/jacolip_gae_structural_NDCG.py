...
parser.add_argument('--dataset', type=str, default='BlogCatalog', help='dataset', choices=["BlogCatalog", "facebook", "Flickr"])
...
def save_log(*args, **kwargs):
    path = 'log/' + str(model_name) + '/' + 'struc/' + str(dataset) + '/'
...
def train_lipgrad(epoch, flag=0, lip_grad=False, u=0.0):
    ...
    if lip_grad == True:
        ...
        out, _, _, _ = model(input, adj_norm)
